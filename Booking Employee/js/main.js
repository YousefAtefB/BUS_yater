...
async function BookingEmployeeSearch() {
	...
	if (
		from.options[from.selectedIndex].value == '--SELECT--' ||
		to.options[to.selectedIndex].value == '--SELECT--'
	) {
...
async function BookingEmployeeBook(tripid) {
	...
	//--------------------------------
		...
		let response = await fetch(
			'http://127.0.0.1:8080/BookingEmployeeBook',
			{
				method: 'POST',
				body: JSON.stringify(dataToSend),
				headers: {
...
function addButtonAction() {
	...
	for (let i = 0; i < arr.length; i++) {
		document.querySelector('#trips').children[
			i
		].children[7].onclick = async function () {
			try {
