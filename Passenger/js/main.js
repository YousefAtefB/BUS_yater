...
async function BookingEmployeeSearch() {
	...
	if (dataToSend.location == '--SELECt--' || dataToSend.destination == '--SELECT--') {
		alert('You must choose valid location');
	}
	//--------------------------------
	else {
		let response = await fetch(
			'http://127.0.0.1:8080/BookingEmployeeSearch',
			{
				method: 'POST',
				body: JSON.stringify(dataToSend),
				headers: {
...
async function addThePassenger(tripid, cardId, value) {
	...
	if (
		parseInt(dataToSend.passengerid) < 0 ||
		parseInt(dataToSend.tripId) < 0 ||
		parseInt(dataToSend.cardId) < 0
	) {
...
async function BookingEmployeeCancel(id) {
	...
	//--------------------------------
		...
		let response = await fetch(
			'http://127.0.0.1:8080/BookingEmployeeCancel',
			{
				method: 'POST',
				body: JSON.stringify(dataToSend),
				headers: {
...
async function addButtonAction() {
	for (let i = 0; i < arr.length; i++) {
		document.querySelector('#trips').children[
			i
		].children[6].onclick = async function () {
			try {
				await BookingEmployeeBook(
					document.querySelector('#trips').children[i].children[1]
						.innerText
				);
			} catch {}
			try {
				await addThePassenger(
					document.querySelector('#trips').children[i].children[1]
						.innerText,
					cardId,
					document
						.querySelector('#trips')
						.children[0].children[5].innerText.slice(6)
				);
			} catch {}
		...
		document.querySelector('#my-trips').children[i].children[2].onclick = async function () {
			try {
				await BookingEmployeeCancel(
					document.querySelector('#bookedtrips').children[i]
						.children[1].innerText
				);
			} catch {}
			try {
				await removeThePassenger(
					document.querySelector('#bookedtripss').children[i]
						.children[1].innerText
				);
			} catch {}
			Trips.classList.add('hide');
